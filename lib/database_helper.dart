...
class DatabaseHelper {
  ...
  static const columnGate = 'gateno';
  ...
  Future<List<Map<String, dynamic>>> queryUnsynchedRecords() async {
    Database? db = await instance.database;
    return await db!
        .rawQuery('SELECT id,gateno,date,status FROM $table WHERE status = 0');
  }
  ...
  Future<List<Map<String, dynamic>>> queryAllRecords() async {
    Database? db = await instance.database;
    return await db!.rawQuery('SELECT id,gateno,date,status FROM $table');
  }
